...
def test_autoplay_with_optimal_strategy():
    ...
    with patch(
        ...
        res = use_case.execute(AutoplayCommand(game_id="a3", strategy="optimal"))
...
def test_autoplay_optimal_handles_exception_gracefully():
    """Test autoplay handles AIOptimalPlayer exceptions gracefully."""
    repo = InMemoryGameRepository()
    board = make_small_board()
    repo.save("a6", board)
    repo.save_history("a6", GameHistory(game_id="a6"))
    ...
    with patch(
        ...
        use_case = AutoplayUseCase(repo)
        res = use_case.execute(AutoplayCommand(game_id="a6", strategy="optimal"))

    assert res.success is False
    assert "optimal solver fail" in res.message


def test_autoplay_optimal_strategy_called():
    """Test that AIOptimalPlayer.solve is called when using optimal strategy."""
    repo = InMemoryGameRepository()
    board = make_small_board()
    repo.save("optimal_called", board)
    repo.save_history("optimal_called", GameHistory(game_id="optimal_called"))
    ...
    with patch(
        ...
        use_case = AutoplayUseCase(repo)
        use_case.execute(AutoplayCommand(game_id="optimal_called", strategy="optimal"))
