...
def should_pick_drop_give_successfully(client, save_board, make_empty_board):
    ...
    assert "game" in data and "robot" in data["game"] and "flowers_collected" in data["game"]["robot"]
...
def test_drop_flower_at_different_position(client, save_board, make_empty_board):
    ...
    resp = client.post(f"/api/games/{game_id}/action", json={"action": "pickFlower", "direction": "south"})
    assert resp.status_code == 200
    data = resp.json()
    ...
    resp = client.post(f"/api/games/{game_id}/action", json={"action": "dropFlower", "direction": "east"})
    assert resp.status_code == 200
    data = resp.json()

    # Verify success
    assert data["success"] is True, f"Drop failed: {data}"
    assert len(data["game"]["robot"]["flowers_collected"]) == 0, "Robot should have no flowers after dropping"
...
def test_pick_and_drop_multiple_flowers_lifo(client, save_board, make_empty_board):
    ...
    resp = client.post(f"/api/games/{game_id}/action", json={"action": "pickFlower", "direction": "east"})
    assert resp.status_code == 200
    data = resp.json()
    ...
    resp = client.post(f"/api/games/{game_id}/action", json={"action": "dropFlower", "direction": "west"})
    assert resp.status_code == 200
    data = resp.json()
    ...
    resp = client.post(f"/api/games/{game_id}/action", json={"action": "dropFlower", "direction": "north"})
    assert resp.status_code == 200
    data = resp.json()
