...
def main():
    ...
    for i in range(args.samples_per_class):
        game_state = generate_game_state()
        # Ensure there's a flower on the board
        if not game_state["board"]["flowers_positions"]:
            game_state["board"]["flowers_positions"] = [{"row": 2, "col": 2}]
    ...
    for i in range(args.samples_per_class):
        game_state = generate_game_state()
        # Ensure there's an obstacle on the board
        if not game_state["board"]["obstacles_positions"]:
            game_state["board"]["obstacles_positions"] = [{"row": 3, "col": 3}]
